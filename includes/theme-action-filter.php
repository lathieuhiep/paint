...
function paint_add_arrow($output, $item, $depth, $args)
{
    if ('primary' == $args->theme_location && $depth >= 0) {
        if (in_array("menu-item-has-children", $item->classes)) {
            $output .= '<span class="sub-menu-toggle"><i class="fa-solid fa-angle-down"></i></span>';
        }
    }
...
function paint_set_query_custom_post_type($query): void
{
    ...
    if (!is_admin() && $query->is_main_query()) {
        ...
        if ( is_post_type_archive('paint_project') || ( $query->is_search && $post_type == 'paint_project' ) ) {
            ...
            if ($post_type == 'paint_project') {
                $query->set('post_type', 'paint_project');
